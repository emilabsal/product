<template lang="pug">
header.header 
  .header-inner.container 
    nuxt-link.header-logo(to="/")
      svg(
        width="40",
        height="37",
        viewBox="0 0 40 37",
        fill="none",
        xmlns="http://www.w3.org/2000/svg"
      )
        path(
          fill-rule="evenodd",
          clip-rule="evenodd",
          d="M3.13307 34.4587C1.40988 34.4587 1.6082e-07 33.0488 1.49105e-07 31.3256L7.46349e-08 20.372L7.88626 20.372L7.88626 20.3598L12.6263 20.3598C13.8482 20.3285 14.8821 19.2633 14.8821 18.01C14.8821 16.7255 13.8168 15.6602 12.5323 15.6602L6.26614 15.6602L6.26614 15.6622L4.26148e-08 15.6622L2.13007e-08 12.5272C9.58531e-09 10.804 1.40988 9.3941 3.13307 9.3941L12.5323 9.3941C14.9134 9.3941 16.9499 10.24 18.6104 11.9319C20.3023 13.5924 21.1482 15.6289 21.1482 18.01C21.1482 20.3598 20.3336 22.3963 18.6731 24.0569C17.0126 25.7174 15.0074 26.5947 12.6576 26.626L6.26614 26.626L6.26614 31.3256C6.26614 33.0488 4.85626 34.4587 3.13307 34.4587Z",
          fill="#02BC6B"
        )
        path(
          fill-rule="evenodd",
          clip-rule="evenodd",
          d="M25.0973 5.7578C28.5588 9.11993 30.303 13.3073 30.303 18.0672C30.303 22.7473 28.6318 26.9307 25.2238 30.3034L20.3851 25.7508C22.5046 23.6533 23.5449 21.128 23.5449 18.0672C23.5449 14.9832 22.4733 12.4619 20.303 10.3538L25.0973 5.7578Z",
          fill="#02897A"
        )
        path(
          fill-rule="evenodd",
          clip-rule="evenodd",
          d="M32.5055 -3.14562e-08C37.4911 4.97781 39.9999 11.1499 39.9999 18.2338C39.9999 25.2071 37.5922 31.3753 32.6884 36.3638L28.0186 31.8009C31.6715 28.085 33.4614 23.5781 33.4614 18.2338C33.4614 12.8358 31.6111 8.33276 27.8787 4.60616L32.5055 -3.14562e-08Z",
          fill="#035B81"
        )
        path(
          fill-rule="evenodd",
          clip-rule="evenodd",
          d="M0 26.6524L6.26614 26.6524L6.26614 31.326C6.26614 33.0492 4.85626 34.4591 3.13307 34.4591C1.40988 34.4591 4.34898e-08 33.0492 3.17744e-08 31.326L0 26.6524Z",
          fill="#028E37"
        )
      span.header-logo-title Product
    .header-right 
      nav.nav 
        ul.nav-list 
          li.nav-item(v-for="(link, index) in links", :key="link") 
            nuxt-link.nav-link(:to="link.link") 
              | {{ link.title }}
      .sign
        ui-button(title="Sign In", :outline="true") 
        ui-button(title="Sign Up") 
        ui-button.theme(
          :icon="true",
          name="theme",
          @button-click="changeTheme"
        ) 
</template>

<script>
export default {
  data() {
    return {
      links: [
        { title: "Product", link: "/" },
        { title: "Customers", link: "/customers" },
        { title: "Prices", link: "/prices" },
        { title: "Resources", link: "/resources" },
      ],
    };
  },
  methods: {
    changeTheme() {
      if (document.querySelector("html").hasAttribute("theme")) {
        document.querySelector("html").removeAttribute("theme");
      } else {
        document.querySelector("html").setAttribute("theme", "dark");
      }
    },
  },
};
</script>

<style lang="sass" scoped>
.header
  padding: 18px 0
  &-inner
    display: flex
    align-items: center
    justify-content: space-between
  &-right
    display: flex
    align-items: center
  &-logo
    display: flex
    align-items: center
    text-decoration: none
    &-title
      font-weight: 700
      font-size: 30px
      line-height: 28px
      margin-left: 16px
      color: var(--button-text)

.nav
  margin-right: 24px
  &-list
    display: flex
    align-items: center
    list-style-type: none
  &-link
    display: inline-block
    text-decoration: none
    margin: 0 14px

.sign
  display: flex
  align-items: center
  gap: 24px

.nuxt-link-exact-active
  font-weight: 600
</style>